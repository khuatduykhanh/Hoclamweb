document.querySelectorAll('.product-img-item').forEach(e => { // bắt lấy 3 cái ảnh
    e.addEventListener('click', () => {
        let img = e.querySelector('img').getAttribute('src')  // khi click vào 3 cái ảnh getAttribute ('src') sẽ lấy đường link của 3 cái ảnh này
        document.querySelector('#product-img > img').setAttribute('src', img) //và để 3 cái ảnh này vào id #product-img > img 
    })
})
document.querySelector('#view-all-description').addEventListener('click', () => {
    ...
    document.querySelector('#view-all-description').innerHTML = content.classList.contains('active') ? 'view less' : 'view all' // clasList.contains kiểm tra xem phần tử này có lớp active chưa nếu có thì id view-all-description thêm view less còn chưa có thì thành view all
})
let products = [
    {
